...
from deepfm_basemodel import BaseModel
import torch_mlu
...
class DeepFM(BaseModel):
            ...
            if epoch % verbose == 0:
                print('epoch %d train loss is %.4f train AUC is %.4f' %
                      (epoch, total_loss_epoch / steps_per_epoch,
                       roc_auc_score(np.concatenate(true_ans), np.concatenate(pred_ans))))
                self.val_auc_logloss(val_input, y_val, batch_size=50000)

            acc = roc_auc_score(np.concatenate(true_ans), np.concatenate(pred_ans))
            # print("\nacc = ", acc, "\t", type(acc), "\n")
            if acc >= best_acc:
                torch.save(model.state_dict(), "./deepfm.pth")
    ...
    def predict(self, test_input, batch_size=256, use_double=False):
        ...
        # 保存量化模型参数
        torch.save(quantized_model.state_dict(), './deepfm_quantize_state.pth')
        print("\nSuccessfully Save Quantized Model\n")
        ...
        with torch.no_grad():
            ...
            for index, x_test in enumerate(test_loader):
                ...
                y_pred = quantized_model_mlu(x_mlu)
                print("\nSuccessfully Predict on MLU:\t", y_pred, "\n")
                pred_ans.append(y_pred)
    ...
    def create_embedding_matrix(self, sparse_feature_columns, feat_sizes, embedding_size, init_std=0.0001,
                                device='cpu'):
